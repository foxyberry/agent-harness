...
### `codex exec`(비대화형)에서는 위 우회가 **선택이 아니라 필수**다

작업 디렉터리에 프로젝트 훅(`.codex/hooks.json`)이 있는데 플래그를 안 주면,
`codex exec` 은 **출력 없이 무한정 멈춘다.** 승인 화면을 띄울 터미널이 없어서 신뢰 대기에
걸린 것으로 보인다.

```bash
codex exec --dangerously-bypass-hook-trust "..."   # 프로젝트 훅이 있으면 이게 기본이다
```
