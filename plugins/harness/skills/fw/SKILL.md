...
- `--from codex` defaults to **the other tool**, for switching tools.
- `--current claude` excludes the current tool's live session **only when its environment session ID matches a project log**. Without a positive match, it hides nothing; inspect the output to avoid treating this invocation as previous work.
- Inspect:
