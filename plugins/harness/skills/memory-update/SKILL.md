...
```
compact_transcript.py <고른 세션 경로> --require-attributed-user
```
...
⚠️ 회고 산출물 PR 은 커밋 메시지에 **`[skip reflect]`** 를 넣는다. 경로 기반 skip rule 이
먼저 걸러주지만, 규칙 파일·부수 파일이 섞이면 새는 적이 있었다(#130). 커밋 메시지는
경로 구성과 무관하게 듣는 **최후 방어선**이다 — 안 붙이면 이 PR 의 회고를 다시 요구하는
루프가 돈다.
