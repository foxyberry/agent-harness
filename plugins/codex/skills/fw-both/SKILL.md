...
(`--current codex`). 그래서 "직전 작업" 자리에 방금 실행한 fw 호출이 잡히지 않는다. 반대 툴은
...
```bash
python3 scripts/handoff.py fw --from both --current codex --project-dir "<지금 작업 중인 사용자 프로젝트 절대경로>" 
```
...
- `--from both` = Claude·Codex 양쪽 로그를 함께 출력. `--current codex` 로 현재 세션 배제.
