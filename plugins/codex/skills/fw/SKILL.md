...
```bash
python3 scripts/handoff.py fw --from claude --current codex --project-dir "<지금 작업 중인 사용자 프로젝트 절대경로>" 
```
...
- `--from claude` 는 **반대 툴**이 기본값이다 — 툴을 갈아탔을 때 쓰는 값이다.
- `--current codex` 는 **지금 이 세션**을 후보에서 뺀다. 이게 없으면 최신 = 방금 켠 세션이라
...
```bash
python3 scripts/handoff.py fw --from codex --current codex --project-dir "<지금 작업 중인 사용자 프로젝트 절대경로>" 
```
