...
```bash
python3 scripts/handoff.py fw --from claude --current codex --project-dir "<absolute-path-to-user-project>"
```
...
- `--from claude` defaults to **the other tool**, for switching tools.
- `--current codex` excludes the current tool's live session **only when its environment session ID matches a project log**. Without a positive match, it hides nothing; inspect the output to avoid treating this invocation as previous work.
- Inspect:
...
```bash
python3 scripts/handoff.py fw --from codex --current codex --project-dir "<absolute-path-to-user-project>"
```
