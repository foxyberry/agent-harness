...
| `handoff-load` | 저장된 핸드오프를 읽고 커밋 여부까지 표시하며 현재 Git 상태와 대조해 작업 재개 |
...
| `template-check` | 프로젝트의 `.claude/memory/` 를 플러그인에 실린 템플릿 참조본과 읽기 전용으로 비교 *(미출시 — 0.12.2 에 없음)* |
...
플러그인은 `project-template/` 자체를 싣지 않습니다. 0.12.2 다음 릴리스부터 그중
`.claude/memory/` 부분만 읽기 전용 참조본으로 실리고, `/template-check` 가 그 파일 중 프로젝트에 없거나 내용이 다른 것을 알려 줍니다.
아무것도 바꾸지 않으며, 내용이 다른 파일은 직접 고친 것일 수도 있습니다. 어느 버전의 템플릿을
복사했는지 기록이 없어서 둘을 구분하지 못합니다.
...
- Claude Code·Codex 양쪽 공개 marketplace 설치 검증 완료 (이전 릴리스에서 측정한 결과이며
  매 버전마다 다시 돌리지는 않습니다)
- 0.12.2 는 양쪽 어댑터에 스킬 7개, 미출시 `template-check` 를 더해 `main` 은 8개. 크로스툴 핸드오프
  검증 완료 (한쪽이 저장한 것을 다른 쪽이 로드)
