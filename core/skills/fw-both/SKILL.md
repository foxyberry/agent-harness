...
**Live-session handling:** `--current {{AGENT}}` excludes the **current tool's live session only when its environment session ID matches a project log**. Without a positive match, it hides nothing. The other tool's latest log remains eligible; do not assume that tool has stopped running. Check the output for this invocation before treating it as prior work.
...
```bash
{{HANDOFF}} fw --from both --current {{AGENT}} {{PROJECT_DIR_ARG}}
```
{{PATH_NOTE}}
- `--from both` includes Claude and Codex logs. `--current {{AGENT}}` enables the matching-based exclusion described above.
